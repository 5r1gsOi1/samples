...
class RandomUniform {
 public:
  ...
  int operator()(const int max) { return operator()(0, max); }
...
auto CreateRandomVector(const size_t length, const int random_min = 1,
                        const int random_max = 9) {
  RandomUniform random{};
  std::vector<int> result{};
  auto generate = [&random, random_min, random_max] {
...
auto CreateRandomMap(const size_t length, const int random_min = 1,
                     const int random_max = 9) {
  RandomUniform random{};
  ...
  auto generate = [&random, random_min, random_max] {
    static int i{-1};
    ++i;
    return std::make_pair(i, random(random_min, random_max));
  };
...
void SynchronizeVectorWithMap(std::vector<int>& vector,
                              std::map<int, int>& map) {
  auto predicate = [&map](const int e) {
    return map.end() ==
           std::find_if(map.begin(), map.end(),
                        [&e](const auto& p) { return p.second == e; });
  };
  auto new_end{std::remove_if(vector.begin(), vector.end(), predicate)};
  vector.erase(new_end, vector.end());
}

void SynchronizeMapWithVector(std::map<int, int>& map,
                              std::vector<int>& vector) {
  auto predicate{[&vector](const auto& p) {
    return vector.end() == std::find(vector.begin(), vector.end(), p.second);
  }};
  auto iter{map.begin()};
  for (; iter != map.end();) {
    if (predicate(*iter)) {
